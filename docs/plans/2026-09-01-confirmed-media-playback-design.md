...
- Two bounded recovery snapshots describe the same source and media item, and the later raw elapsed position is at least 0.5 seconds ahead of the earlier position. The accepted state is normalized to `isPlaying == true` because advancing position is the confirmation signal.
...
5. A later matching paused snapshot is accepted only if its elapsed position advanced by at least 0.5 seconds.
