  ...
  private func runInventory(
    ...
    while !Task.isCancelled {
      do {
        ...
        let listed: [T3ConnectEnvironment]
        do {
          listed = try await relay.listEnvironments(accountToken: record.accessToken)
        } catch T3ClientError.unauthorized {
          // An account-level rejection from the inventory endpoint means the saved OAuth grant
          // must be linked again. Environment authorization failures remain scoped to their
          // individual cloud monitor and are not normalized here.
  ...
  private func runCloudMonitor(
    ...
    while !Task.isCancelled {
      do {
        ...
        failures += 1
        upsertCloudCandidate(
          T3CodeActivity.retainingStalePayload(
            Self.offlineSnapshot(environment, error: error), from: cloudCandidates))
        onCloudCycleCompleted(environment.environmentID)
        do {
          try await sleeper.sleep(
            for: T3CodeActivity.reconnectDelay(failureCount: failures, remote: true))
