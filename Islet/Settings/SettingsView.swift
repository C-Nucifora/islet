  ...
  private var pulseForm: some View {
    Form {
      Section("Pulse providers") {
        ...
        .font(.caption).foregroundStyle(.secondary)
        Text(
          "Local scripts publish status and web actions over \(pulseServer.listeningAddress ?? "127.0.0.1:47717"). Each approved provider has its own source identity and permissions. Credentials stay in user-only files and are never included in diagnostics."
        )
        .font(.caption).foregroundStyle(.secondary)
