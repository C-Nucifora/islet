...
final class T3ConnectPresentationTests: XCTestCase {
  ...
  func testEnvironmentRowsExplainSourceStateAndAvailableControls() {
    ...
    for item in cases {
      ...
      XCTAssertEqual(row.accessibilityLabel, "Studio, \(item.sourceText), Offline: Timed out")
    }
  }
  ...
  func testStaleEnvironmentRowPreservesVisibleAndSpokenFreshnessCues() {
    let row = T3EnvironmentRowPresentation(
      snapshot: environment(
        source: .connect, state: .offline("Timed out"), isStale: true))

    XCTAssertEqual(row.stateText, "Offline")
    XCTAssertTrue(row.isStale)
    XCTAssertEqual(
      row.accessibilityLabel,
      "Studio, T3 Connect, Offline: Timed out, showing the last update")
  }

  func testHiddenStaleManualRowPreservesVisibleAndSpokenFreshnessCues() {
    let row = T3EnvironmentRowPresentation(
      manualLabel: "Studio", profileEnabled: true, monitoringEnabled: true,
      state: .offline("Timed out"), isStale: true)

    XCTAssertEqual(row.stateText, "Offline")
    XCTAssertTrue(row.isStale)
    XCTAssertEqual(
      row.accessibilityLabel,
      "Studio, Manually paired, Offline: Timed out, showing the last update")
  }
  ...
  private func environment(
    ...
    T3EnvironmentSnapshot(
      ...
      serverVersion: "1", state: state, agents: [], isStale: isStale)
  }
}
